...
/// `--force` is the way past that refusal without a terminal, and without `-o`
/// naming the same file twice.
#[test]
fn emit_force_writes_over_a_hand_written_nsi() {
    let fixtures = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("fixtures");
    let script = fixtures.join("emit-force.lua");
    let written = fixtures.join("emit-force.nsi");
    std::fs::write(&script, CLEAN).expect("write the program");
    std::fs::write(&written, "Name \"by hand\"\n").expect("write the original");

    let forced = Command::new(PathBuf::from(env!("CARGO_BIN_EXE_installua")))
        .arg("emit")
        .arg("--force")
        .arg(&script)
        .current_dir(&fixtures)
        ...
        .status
        .success();
    let now = std::fs::read_to_string(&written).unwrap_or_default();
    let _ = std::fs::remove_file(&script);
    let _ = std::fs::remove_file(&written);

    assert!(forced, "`emit --force` refused a hand-written script");
    assert!(
        now.starts_with("; Generated by installua."),
        "`emit --force` left the original:\n{now}"
    );
}
