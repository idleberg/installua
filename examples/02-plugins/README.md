...
`.r0` writes a register the AST does not know about — that is why the design calls
...
**The string-template model makes caller-saves much rarer than expected.** `target` and
...
`.onInit` reads it. Assignment declares a global, but that does not say where
