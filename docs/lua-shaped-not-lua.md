...
| floats — `1.5`, `1e3`, `/`, `^` | rejected **at the literal** | NSIS has no float arithmetic at all. `/` says *use `//`*; `^` is rejected outright because NSIS's `^` is xor and silently remapping it ships a wrong installer |
...
rule to be *about*: every top-level name is resolved before any body is lowered, and the
...
literal is *data*, never a template — see the other table for why.
...
dies *silently* — no dialog, no log line, no error level. The compiler warns on unbounded
