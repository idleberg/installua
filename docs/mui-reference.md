<!-- Hand-written. Not generated yet — see "Generating this file" in
     [reference-map.md](reference-map.md). Every spelling below is copied from
     `src/mui/rows.rs` and `src/lower/mod.rs`, so the entries are true as of the
     current tables. -->

# MUI2 Reference

The MUI2 half of the [Installua reference](reference-map.md): the eight pages,
...
its controls — is in [reference-map.md](reference-map.md).
...
`instFiles`, `finish`, `confirm` — plus [`custom`](reference-map.md#windows-and-controls).
