...
//! Phase 2's exit criterion. The MUI2 page tables the declaration walk reads
//! are in [`pages`], which is data rather than lowering and is edited when MUI2
//! changes rather than when this phase does.
//!
...
pub use pages::{mui_defines, v1_installer_fields, v1_page_surface};
...
/// errors belong to [`Lowerer::custom_page`], which reports them once where the
/// page is lowered; this is the claim pass looking for the bare names inside.
fn page_controls(value: &Expr) -> Option<&[TableField]> {
    table_arg(value, "controls")
}
...
/// where the group is lowered; this is the claim pass looking for the bare names
/// inside.
fn group_members(value: &Expr) -> Option<&[TableField]> {
    if let Expr::Call { args, .. } = value
        && let [_, Expr::Table { fields, .. }] = args.as_slice()
    {
        return Some(fields);
    }
    table_arg(value, "sections")
}

/// The table under `key` in a call's single table argument: `call { key = { … }
/// }`. Both readers above want the same three unwraps, and both are claim
/// passes, so a shape that does not match is `None` rather than a diagnostic.
fn table_arg<'e>(value: &'e Expr, key: &str) -> Option<&'e [TableField]> {
...
/// [`pages::v1_page_surface`] is frozen: the stub's block classes are
/// hand-written, and only a test reading both keeps them from drifting.
const V1_BLOCKS: &[&str] = &[
    "attributes",
    "installer",
    "uninstaller",
