...
///
/// Last, not first: `assemble` maps `makensis` line numbers back to the source,
/// and a line appended after the script shifts none of them. The CLI adds it,
/// not the library, so `--stdout` and the goldens stay exactly the lowering.
const GENERATED: &str = "; Generated by installua. Edit the .lua, not this file.";

/// Refuse to write over a `.nsi` that does not end in [`GENERATED`], unless a
...
fn guard_hand_written(path: &Path) -> Stop {
    ...
    if existing.trim_end().ends_with(GENERATED) {
        return Ok(());
    }
