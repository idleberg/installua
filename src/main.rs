...
fn build(args: &BuildArgs, stdout: bool, assemble: bool) -> ExitCode {
    ...
    let Some((nsi, mut map)) = result else {
    ...
    let nsi = format!("{GENERATED}\n{nsi}");
    map.prepend(installua::map::Origin::Emitted("generated marker"));
...
/// The last line of every `.nsi` the CLI writes, and how the next build tells
/// its own output from a script someone wrote by hand.
///
/// First, where a human opening the file reads it; the line map gets a matching
/// entry in front, so `makensis` line numbers still land on the right source.
/// The CLI adds it, not the library, so `--stdout` and the goldens stay exactly
/// the lowering.
const GENERATED: &str = "; Generated by installua. Edit the .lua, not this file.";
...
fn guard_hand_written(path: &Path) -> Stop {
    ...
    if existing.starts_with(GENERATED) {
        return Ok(());
    }
    ...
    clark::intro("File exists");
    let overwrite = clark::confirm(format!(
        "{} was not written by installua. Overwrite?",
        path.display()
    ))
    .initial_value(false)
    .interact()
    ...
    if overwrite {
        clark::outro("overwriting");
        Ok(())
    } else {
        clark::cancel("left alone");
