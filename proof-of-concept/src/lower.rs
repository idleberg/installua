//! Phase 3: AST -> IR. Overlay-driven, so the ergonomic decisions (§6) live in
...
/// `$0`-`$9` then `$R0`-`$R9` (§3). Locals are handed out from the front,
/// expression temporaries from the back, so the two never collide without a
/// liveness pass — which is what §9-3 replaces this with.
...
pub fn lower(program: &Program, diags: &mut Diagnostics) -> ir::Module {
    ...
    if let Some(installer) = &program.installer {
        ...
        for spec in overlay::INSTALLER_FIELDS {
            ...
            // Derived argument (§6): `Name` takes an ampersand-doubled twin,
            // emitted only when the string actually contains `&`.
    ...
    // The preprocessor is textual and sequential (§12), so these keep source
    // order and each `pre.*` command sits immediately above the `!define` that
    // names its output.
    ...
    // boundary, and neither does anything else here (§8).
...
impl Lowering<'_> {
    ...
    /// `${MacroName} <inputs…> [$dest]` — where the output goes is a property of
    /// the macro, not a convention (§6, §7-4).
    fn macro_call(
        &mut self,
        call: &MacroCall,
    ...
    /// The jump-table half of `MessageBox` (§13): one instruction, then a block
    /// per handled button. `IDxxx label` is the same fused-branch trick as `if`
    ...
    fn local(&mut self, local: &Local, out: &mut Vec<Item>) {
        ...
        let ty = match &local.value {
            // Fold first (§4): `96 * 1024` never reaches `IntOp`.
    ...
    /// Conditions are fused into the branch targets of a single `IntCmp` (§8);
    /// no boolean is ever materialized into a register.
    ...
    fn int_operand(&mut self, expr: &Expr, out: &mut Vec<Item>) -> Option<Arg> {
        ...
        if value.ty != Ty::Int {
            self.diags.push(
                ...
                .with_note("arithmetic and comparisons are integer-only in this PoC (§3)"),
            );
            return None;
        }
        Some(value.arg)
    ...
    /// (§4) — the concatenation happens in the emitted literal.
    ...
    fn expr(&mut self, expr: &Expr, out: &mut Vec<Item>) -> Option<Value> {
        ...
        match expr {
            ...
            // `IntOp` operand, a `string` one has to be quoted (§7-1).
...
/// Constant folding (§4, §7-2). Only literals fold: a `<const>` deliberately
/// stays `${NAME}` in the output, so the `!define` remains the thing a reader
/// edits.
