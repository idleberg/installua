...
| a config file for `Name`, `OutFile`, …                     | anything NSIS expresses as a script attribute belongs in `attributes {}` | `installua.toml` is not a settings file: it lists the projects and bounds the declaration search, and nothing else |
